...
export default function HistoryScreen() {
  ...
  if (isLoading) {
    return (
      <View style={{ flex: 1, backgroundColor: colors.appBackground, paddingTop: insets.top + SPACING.lg, paddingHorizontal: SPACING.screenH }}>
        <Pressable onPress={() => router.back()} style={{ marginBottom: SPACING.md, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <Ionicons name="arrow-back" size={20} color={colors.textMuted} />
          <Text style={{ fontSize: FONT_SIZES.sm, fontFamily: FONTS.sansRegular, color: colors.textMuted }}>Back</Text>
        </Pressable>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.appBackground }}>
      <View style={{ flex: 1 }}>
        <View style={{ paddingTop: insets.top + SPACING.lg, paddingHorizontal: SPACING.screenH }}>
          {/* Back button */}
          <FadeInView delay={0} duration={300}>
            <Pressable onPress={() => router.back()} style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: SPACING.md }}>
              <Ionicons name="arrow-back" size={20} color={colors.textMuted} />
              <Text style={{ fontSize: FONT_SIZES.sm, fontFamily: FONTS.sansRegular, color: colors.textMuted }}>Back</Text>
            </Pressable>
          ...
          <FadeInView delay={100} duration={300}>
            <View style={styles.statRow}>
              {[
                { icon: 'book' as const, color: colors.primary, value: totalTests, label: 'Tests Taken' },
                { icon: 'trending-up' as const, color: colors.warning, value: `${avgScore}%`, label: 'Avg Score' },
                { icon: 'trophy' as const, color: colors.warning, value: `${bestScore}%`, label: 'Best Score' },
              ].map((stat) => (
        ...
        {filteredTests.length === 0 ? (
          ...
          <FlatList
            ...
            contentContainerStyle={{
              ...
              paddingBottom: 40 + insets.bottom + SPACING.xl,
              gap: 10,
            }}
            renderItem={({ item }) => {
                  ...
                  <View style={styles.testCardInner}>
                    <View style={{ flex: 1, marginRight: 12 }}>
                      ...
                      <View style={styles.badgeRow}>
                        <Badge text={item.difficulty ?? 'mixed'} color={colors.textMuted} />
                        {item.timeTaken ? (
                          <Badge text={`${Math.round(item.timeTaken / 60)}m`} color={colors.textMuted} />
                        ) : null}
                      </View>
...
const styles = StyleSheet.create({
  ...
  testCardInner: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  testTopic: {
  ...
  scoreDetail: { fontSize: FONT_SIZES.sm, fontFamily: FONTS.sansRegular, lineHeight: FONT_SIZES.sm * 1.5 },
});
